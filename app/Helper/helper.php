...
{
    $base = \Carbon\Carbon::parse($plan_start_date);
    $t = strtolower(trim((string) $plan_type));
    $left_days = (int) $left_days;
    $plan_duration = (int) $plan_duration ?: 1;

    if ($t === 'weekly') {
        $freePlan = \App\Models\Plans::where('identifier', 'free')->first();
        $trialDays = $freePlan ? (int) $freePlan->trial_period : 7;
        $days = $left_days + max(1, $trialDays);

        return $base->copy()->addDays($days)->format('Y-m-d H:i:s');
    }
    if ($t === 'monthly') {
        return $base->copy()->addMonths($plan_duration)->addDays($left_days)->format('Y-m-d H:i:s');
    }
    if ($t === 'yearly') {
        return $base->copy()->addYears($plan_duration)->addDays($left_days)->format('Y-m-d H:i:s');
    }

    return $base->copy()->addDays(7)->format('Y-m-d H:i:s');
}

/**
 * If computed end is missing or not strictly after start, extend from start (prevents invalid ranges in UI / logic).
 */
function subscription_end_at_or_fix(string $startAt, string $endAt): string
{
    ...
    return $start->copy()->addWeek()->format('Y-m-d H:i:s');
}

/**
 * Rows in `plans` with this `module` are the single billing catalog (prices / titles like Free, Basic, Premium).
 * Subscriptions still set `provider_subscriptions.module` to service | ecommerce | classified for limits and UI context.
 */
function subscription_billing_plan_module(): string
{
    return 'service';
}

/**
 * Subscriptions that count as “active” for limits: status active and not past end_at.
 */
function provider_subscriptions_valid_query(int $user_id, ?string $module = null)
{
    $query = \App\Models\ProviderSubscription::query()
...
function is_any_plan_active($user_id, $module = null)
{
    $user_subscribed = provider_subscriptions_valid_query((int) $user_id, $module)->first();

    return $user_subscribed ? 1 : 0;
}
...
function plan_limit_checkbox_on($value): bool
{
...
function plan_limit_has_numeric_cap($limit): bool
{
    if ($limit === null || $limit === '') {
        return false;
    }
...
/**
 * Use first non-empty limitation row; avoids "featured_*" => [] leaving limits stuck at not-in-plan.
 */
function plan_limit_resolve_row(array $planLimit, string $primaryKey, ?string $fallbackKey = null): array
{
    foreach (array_filter([$primaryKey, $fallbackKey]) as $key) {
        $row = $planLimit[$key] ?? null;
        if (is_array($row) && count($row) > 0) {
            return $row;
        }
    }

    return ['is_checked' => 'on', 'limit' => null];
}

/**
 * Turn get_provider_plan_limit() into a user-facing error string, or null when the action is allowed.
 * 'inactive' / null — no subscription for that module or plan expired (block).
 * 0 — within plan. '' — feature not included in plan. 1 — numeric cap exceeded.
 */
function plan_limit_user_message($exceed, string $name): ?string
{
    if ($exceed === 'inactive' || $exceed === null) {
        return __('messages.active_plan_required_for', ['name' => $name]);
    }
    if ($exceed === 0 || $exceed === '0') {
        return null;
    }
    if ((int) $exceed === 1) {
        return __('messages.limit_exceed', ['name' => $name]);
    }
    if ($exceed === '') {
        return __('messages.not_in_plan', ['name' => $name]);
    }
...
function get_provider_plan_limit($provider_id, $type)
{
    $limit_array = array();
    $module = 'service';
    if ($type === 'ecommerce' || $type === 'featured_ecommerce') {
        $module = 'ecommerce';
    } elseif ($type === 'classified' || $type === 'featured_classified') {
        $module = 'classified';
    }
    ...
    if ($hasActivePlan) {
        ...
        if ($planKind === 'unlimited') {
            $exceed = 0;
        } elseif (($planKind === 'limited' || $planKind === '') && !empty($get_current_plan->plan_limitation)) {
            $rawLimitation = $get_current_plan->plan_limitation;
            $get_plan_limit = is_array($rawLimitation)
                ? $rawLimitation
                : json_decode($rawLimitation ?? '', true);
            if (!is_array($get_plan_limit)) {
                $get_plan_limit = [];
            }
            ...
            if ($type === 'ecommerce') {
                $limit_array = plan_limit_resolve_row($get_plan_limit, 'ecommerce', 'service');
                $productCount = \App\Models\Product::where('provider_id', $provider_id)->where('service_type', 'ecommerce')->whereDate('created_at', '>=', $plan_start_date)->count();
                if (plan_limit_checkbox_on($limit_array['is_checked'] ?? 'off')) {
                    if (plan_limit_has_numeric_cap($limit_array['limit'] ?? null)) {
                        $exceed = $productCount >= (int) $limit_array['limit'] ? 1 : 0;
                    } else {
                        $exceed = 0;
                    }
                }
            }
            if ($type === 'featured_ecommerce') {
                $limit_array = plan_limit_resolve_row($get_plan_limit, 'featured_ecommerce', 'featured_service');
                $featuredProductCount = \App\Models\Product::where('provider_id', $provider_id)
                ...
                if (plan_limit_checkbox_on($limit_array['is_checked'] ?? 'off')) {
                    if (plan_limit_has_numeric_cap($limit_array['limit'] ?? null)) {
                        $exceed = $featuredProductCount >= (int) $limit_array['limit'] ? 1 : 0;
                    } else {
                        $exceed = 0;
                    }
                }
            ...
            if ($type === 'classified') {
                $limit_array = plan_limit_resolve_row($get_plan_limit, 'classified', 'service');
                $postCount = \App\Models\Post::where('provider_id', $provider_id)->where('service_type', 'classified')->whereDate('created_at', '>=', $plan_start_date)->count();
                if (plan_limit_checkbox_on($limit_array['is_checked'] ?? 'off')) {
                    if (plan_limit_has_numeric_cap($limit_array['limit'] ?? null)) {
                        $exceed = $postCount >= (int) $limit_array['limit'] ? 1 : 0;
                    } else {
                        $exceed = 0;
                    }
                }
            }
            if ($type === 'featured_classified') {
                $limit_array = plan_limit_resolve_row($get_plan_limit, 'featured_classified', 'featured_service');
                $featuredPostCount = \App\Models\Post::where('provider_id', $provider_id)
                ...
                if (plan_limit_checkbox_on($limit_array['is_checked'] ?? 'off')) {
                    if (plan_limit_has_numeric_cap($limit_array['limit'] ?? null)) {
                        $exceed = $featuredPostCount >= (int) $limit_array['limit'] ? 1 : 0;
                    } else {
                        $exceed = 0;
                    }
                }
            ...
            // Limited plan with no JSON, unknown plan_type, or non-limited without applicable payload — treat as no cap.
            $exceed = 0;
        }
    } else {
        return 'inactive';
