...
class CommanController extends Controller
{
    ...
    public function getSearchList(Request $request)
    {
        ...
        if ($request->boolean('include_modules')) {
            ...
            if ($lat !== null && $lat !== '' && $lng !== null && $lng !== '') {
                try {
                    $matchedZoneIds = $this->getMatchingZonesByLatLng($lat, $lng);
                } catch (\Throwable $e) {
                    $matchedZoneIds = [];
                }
            }
            ...
            $serviceQuery = Service::query()
                ...
                ->with('translations')
                ->where(function ($query) use ($keyword, $headerValue) {
                    $query->where('name', 'like', "%{$keyword}%")
                        ->orWhereHas('translations', function ($translationQuery) use ($keyword, $headerValue) {
                            $translationQuery->where('attribute', 'name')
                                ->where('locale', $headerValue)
                                ->where('value', 'like', "%{$keyword}%");
                        });
                });
            if (!empty($matchedZoneIds)) {
                $serviceQuery->whereHas('serviceZoneMapping', function ($q) use ($matchedZoneIds) {
                    $q->whereIn('zone_id', $matchedZoneIds);
                });
            }
            $services = $serviceQuery->latest()->take(20)->get();
            ...
            if (!empty($matchedZoneIds)) {
                $productQuery->whereHas('productZoneMapping', function ($q) use ($matchedZoneIds) {
                    $q->whereIn('zone_id', $matchedZoneIds);
                });
            }
            $products = $productQuery->latest()->take(20)->get();
            ...
            if (!empty($matchedZoneIds)) {
                $postQuery->whereHas('postZoneMapping', function ($q) use ($matchedZoneIds) {
                    $q->whereIn('zone_id', $matchedZoneIds);
                });
            }
            $posts = $postQuery->latest()->take(20)->get();

            $normalized = collect()
                ->merge($services->map(function ($item) use ($headerValue) {
                    $name = $item->name;
                    if ($headerValue !== 'en') {
                        $name = $item->translate('name', $headerValue) ?: $name;
                    }
                    return [
                        'id' => (int) $item->id,
                        'name' => $name,
                        'module' => 'service',
                        'url' => route('service.detail', $item->id),
                        'image' => getSingleMedia($item, 'service_attachment', null),
                        'created_at' => optional($item->created_at)->timestamp ?? 0,
                    ];
                }))
                ->merge($products->map(function ($item) use ($headerValue) {
                    $name = $item->name;
                    if ($headerValue !== 'en') {
                        $name = $item->translate('name', $headerValue) ?: $name;
                    }
                    return [
                        ...
                        'name' => $name,
                        'module' => 'product',
                        'url' => route('product.detail', $item->id),
                        'image' => getSingleMedia($item, 'product_attachment', null),
                        'created_at' => optional($item->created_at)->timestamp ?? 0,
                    ];
                }))
                ->merge($posts->map(function ($item) use ($headerValue) {
                    $name = $item->name;
                    if ($headerValue !== 'en') {
                        $name = $item->translate('name', $headerValue) ?: $name;
                    }
                    return [
                        ...
                        'name' => $name,
                        'module' => 'post',
                        'url' => route('post.detail', $item->id),
                        'image' => getSingleMedia($item, 'post_attachment', null),
                        'created_at' => optional($item->created_at)->timestamp ?? 0,
                    ];
                }))
            ...
            return comman_custom_response(['data' => $normalized]);
        }

        if ($request->has('provider_id')) {
            $service = Service::where('status', 1)->where('service_type', 'service')->with(['providers', 'category', 'serviceRating', 'shops','zones','translations'])->orderBy('created_at', 'desc');
        } else {
            $service = Service::where('status', 1)->where('service_request_status', "approve")->where('service_type', 'service')->with(['providers', 'category', 'serviceRating', 'shops','zones','translations'])->orderBy('created_at', 'desc');
        }
        if ($request->has('request_status')) {

            if ($request->request_status == 'reject') {
                $service->where('service_request_status', operator: "reject");
        ...
        if ($request->has('search') && !empty($request->search)) {
            $service->where(function ($query) use ($request, $headerValue) {
                $query->where('name', 'like', "%{$request->search}%")
                    ->orWhereHas('translations', function ($translationQuery) use ($request, $headerValue) {
                        $translationQuery->where('attribute', 'name')
                                        ->where('locale', $headerValue)
        ...
        if ($request->has('shop_id') && $request->shop_id != '') {
            $service->whereHas('shops', function ($a) use ($request) {
                $a->whereIn('shop_id', explode(',', $request->shop_id));
            });
        }

        if($request->has('zone_id') && $request->zone_id != ''){
            $service->whereHas('zones', function ($query) use ($request) {
                $query->whereIn('service_zones.id', explode(',', $request->zone_id));
            });
        }

        if($request->has('request_status') && $request->request_status == 'on_shop'){
              $service->where('service_request_status', 'approve');
          }
    ...
    public function downloadInvoice(Request $request)
    {
        ...
        $bookingdata = Booking::with('handymanAdded', 'payment', 'bookingExtraCharge', 'bookingPackage', 'shop')->where('id', $booking_id)->first();
