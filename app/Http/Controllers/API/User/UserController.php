...
class UserController extends Controller
{
    ...
    public function register(UserRequest $request)
    {
        ...
        if ($request->provider_id !== null && $request->id == null && default_earning_type() === 'subscription') {
            if (!empty($input['provider_id'] && $input['user_type'] === 'handyman')) {
                $handymanMsg = plan_limit_user_message(get_provider_plan_limit($input['provider_id'], 'handyman'), __('messages.handyman'));
                if ($handymanMsg !== null) {
                    if ($request->is('api/*')) {
                        return comman_message_response($handymanMsg);
                    }

                    return redirect()->back()->withErrors($handymanMsg);
                }
            }
        }
        ...
        if ($user) {
            ...
            if (!empty($user->referred_by)) {
                ...
                $referral_rule = LoyaltyReferralRule::where('status', 1)
                ...
                if ($referral_rule) {
                    ...
                    $referral_count =  LoyaltyPointActivity::where('related_id', $user->referred_by)
                        ->where('source', 'referral_signup')
                        ->count(); // count = 0 if never used before

                    if ($referral_count < $referral_rule->max_referrals_per_user) {
                        // Credit points to new user
                        LoyaltyPointActivity::create([
                            'user_id' => $user->id,
                            'type' => 'referral',
                            'points' => $referral_rule->referred_user_points,
                            'earn_type' => 'credit',
                            'source' => 'referral_signup',
                            'description' => 'Points for signing up via referral',
                            'related_id' => $user->referred_by,
                        ]);

                        // Increment new user's loyalty points
                        $user->increment('loyalty_points', $referral_rule->referred_user_points);

                        // Credit points to referrer
                        LoyaltyPointActivity::create([
                            'user_id' => $user->referred_by,
                            'type' => 'referral',
                            'points' => $referral_rule->referrer_points,
                            'earn_type' => 'credit',
                            'source' => 'referral_bonus',
                            'description' => 'Points for referring new user',
                            'related_id' => $user->id,
                        ]);

                        // Increment referrer's loyalty points
                        $referrer = User::find($user->referred_by);

                        if($referrer){
                            $referrer->increment('loyalty_points', $referral_rule->referrer_points);
                        }
                    }
                }
            }
    ...
    public function validateReferralCode(Request $request)
    {
        ...
        if (!$referrer) {
            return response()->json([
                'status' => false,
                'message' => 'Oops! This referral code doesn’t exist.'
            ]);
        }

        // Get the active referral rule
        $referral_rule = LoyaltyReferralRule::where('status', 1)->first();

        if ($referral_rule) {
            // Count how many users have already used this referrer’s code
            $referral_count = User::where('referred_by', $referrer->id)->count();
        ...
        return response()->json([
            'status' => true,
            'message' => 'Success! Your referral is good to go!'
        ]);
    }
    ...
    public function userDetail(Request $request)
    {
        ...
        if ($user->user_type == 'provider') {
            ...
            // Shops for this provider
            $shops = Shop::where('provider_id', $id)
                ->where('is_active', 1)
                ->orderBy('id', 'desc')
                ->get();
            $shops = ShopResource::collection($shops);

            // Handyman ratings for provider (if any)
            $handyman_rating = HandymanRating::where('handyman_id', $id)
                ->orderBy('id', 'desc')
                ->paginate(10);
            $handyman_rating = HandymanRatingResource::collection($handyman_rating);

            // Handyman staff
            $handyman_staff = User::where('user_type', 'handyman')
            ...
            if (!empty($handyman_staff)) {
                foreach ($handyman_staff as $image) {
                    $profile_array[] = $image->login_type !== null
                        ? $image->social_image
                        : getSingleMedia($image, 'profile_image', null);
                }
            }
        }
