...
class DashboardController extends Controller
{
    ...
    public function dashboardDetail(Request $request)
    {
        ...
        $shopQuery = Shop::query()->where('is_active', 1);

        // Apply city filter
        if ($request->has('city_id') && !empty($request->city_id)) {
            $shopQuery->where('city_id', $request->city_id);
        }
        ...
        if (
            ...
            try {
                ...
                \Log::error('Shop location filter error: ' . $e->getMessage());
            }
        } else {
            $shopQuery->orderBy('id', 'desc');
        }
        ...
        $referral = $referralRule ? true : false;
    ...
    public function adminDashboard(Request $request)
    {
        ...
        $response = [
            ...
            'total_product'                 => Product::myProduct()->count(),
