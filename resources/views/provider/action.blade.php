...
{{-- <a class="me-2" href="{{ route('provider.time-slot',['id' => $provider->id]) }}" title="{{ __('messages.My_time_slot',['form' => __('messages.provider') ]) }}"><i class="fa fa-clock text-primary "></i></a> --}}
    ...
    @if($auth_user->hasAnyRole(['admin','demo_admin']) || $auth_user->can('plan add'))
    <a class="me-2" href="{{ route('admin.subscription.extend',['user_id' => $provider->id]) }}" title="Extend Subscription"><i class="fa fa-calendar-plus text-warning"></i></a>
    @endif
