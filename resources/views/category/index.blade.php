<x-master-layout>
                        ...
                        <div class="d-flex justify-content-between align-items-center p-3 flex-wrap gap-3">
                            <div>
                                ...
                                @if (!empty($moduleContext))
                                    <span class="badge bg-primary mt-1">{{ __('messages.category_module') }}:
                            ...
                            <div class="d-flex flex-wrap gap-2 align-items-center">
                                @if ($auth_user->can('category add'))
                                    @if (!empty($moduleContext))
                                        <a href="{{ route('category.create', ['module' => $moduleContext]) }}" class="btn btn-sm btn-primary"><i class="fa fa-plus-circle"></i> {{ __('messages.new') }}</a>
                                        <a href="{{ route('category.index') }}" class="btn btn-sm btn-outline-secondary">{{ __('messages.all') }}</a>
                                    @else
                                        <a href="{{ route('category.create', ['module' => 'service']) }}" class="btn btn-sm btn-primary"><i class="fa fa-plus-circle"></i> {{ __('messages.services') }}</a>
                                        <a href="{{ route('category.create', ['module' => 'ecommerce']) }}" class="btn btn-sm btn-info text-white"><i class="fa fa-plus-circle"></i> {{ __('messages.ecommerce') }}</a>
                                        <a href="{{ route('category.create', ['module' => 'classified']) }}" class="btn btn-sm btn-secondary"><i class="fa fa-plus-circle"></i> {{ __('messages.classifieds') }}</a>
                                    @endif
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="col-md-12">
                            <form action="{{ route('category.bulk-action') }}" id="quick-action-form"
                                ...
                                <button id="quick-action-apply" class="btn btn-primary" data-ajax="true"
                                    data--submit="{{ route('category.bulk-action') }}" data-datatable="reload"
    ...
    @if (!empty($moduleContext))
        <input type="hidden" id="category_list_module" value="{{ $moduleContext }}">
    @endif
    <script>
                ...
                ajax: {
                    ...
                    "data": function(d) {
                        ...
                        d.filter = {
                            ...
                            module_type: $('#category_list_module').val() || ''
                        };
                        d.id = "{{ $id }}";
                        d.type = "{{ $type }}";
                    },
