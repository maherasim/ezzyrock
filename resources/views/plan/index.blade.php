                ...
                <div class="card card-block card-stretch">
                    <div class="card-body p-0">
                        <div class="d-flex justify-content-between align-items-center p-3 flex-wrap gap-3">
                            ...
                            <a href="{{ route('plans.create', ['module' => $module ?? ($filter['module'] ?? 'service')]) }}" class=" float-end me-1 btn btn-sm btn-primary"><i
                                    class="fa fa-plus-circle"></i>
                                {{-- {{ trans('messages.add_form_title', ['form' => trans('messages.plan')]) }} --}}
                                {{ __('messages.new') }}
                            </a>
                        </div>
                    </div>
        ...
        <div class="card-body">
            <div class="row justify-content-between gy-3">
                <div class="col-md-6 col-lg-4 col-xl-3">
                    <div class="col-md-12">
                        <form action="{{ route('plans.bulk-action') }}" id="quick-action-form"
                            ...
                            <button id="quick-action-apply" class="btn btn-primary" data-ajax="true"
                                data--submit="{{ route('plans.bulk-action') }}" data-datatable="reload"
                                data-confirmation='true' data-title="{{ __('plans', ['form' => __('plans')]) }}"
                    ...
                    <div class="d-flex align-items-center gap-3 justify-content-end">
                        <div class="d-flex justify-content-end gap-3">
                            <div class="datatable-filter ml-auto">
                                <input type="hidden" id="module_filter" value="{{ $module ?? ($filter['module'] ?? 'service') }}">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', (event) => {
            ...
            window.renderedDataTable = $('#datatable').DataTable({
                ...
                ajax: {
                    "type": "GET",
                    "url": '{{ route('plans.index_data', ['module' => $module ?? ($filter['module'] ?? 'service')]) }}',
                    "data": function(d) {
                        d.search = {
                            value: $('.dt-search').val()
                        };
