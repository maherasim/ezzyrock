...
(function($) {
                                    ...
                                    if(response.image != null){
                                        ...
                                        if (jQuery.inArray(response.preview, ["service_attachment_preview"]) !== -1) {
                                            ...
                                            if(total_file == 0){
                                                ...
                                                clearFileInput("service_attachment[]", "service_attachment_label");
                                        ...
                                        if (jQuery.inArray(response.preview, ["subcategory_image_preview"]) !== -1) {
                                            $('#'+response.preview+"_"+response.id).remove()
                                             resetFileUI('subcategory_image_preview','subcategory_image');
                                        }
                                        if (jQuery.inArray(response.preview, ["serviceaddon_image_preview"]) !== -1) {
                                            $('#'+response.preview+"_"+response.id).remove()
                                            resetFileUI('serviceaddon_image_preview','serviceaddon_image');
                                        }
                                        if (jQuery.inArray(response.preview, ["slider_image_preview"]) !== -1) {
                                            $('#'+response.preview+"_"+response.id).remove()
                                            resetFileUI('slider_image_preview','serviceaddon_image');
                                        }
    ...
    /**
     * Reset file preview, input, buttons based on preview type
     */
    function resetFileUI(previewId, fileInputName) {
    ...
    function clearFileInput(fileInputName, labelId) {
        const fileInput = document.querySelector(`input[name="${fileInputName}"]`);
        ...
        if (label) {
            label.textContent = "{{ __('messages.choose_file',['file' => __('messages.image')]) }}";
        }
    }
