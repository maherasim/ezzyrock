<x-master-layout>
                ...
                <div class="card card-block card-stretch">
                    <div class="card-body p-0">
                        <div class="d-flex justify-content-between align-items-center p-3 flex-wrap gap-3">
                            ...
                            @if ($auth_user->can('product add') && ! $auth_user->hasAnyRole(['admin', 'demo_admin']))
                                <a href="{{ route('product.create') }}" class="float-end me-1 btn btn-sm btn-primary">
                                    <i class="fa fa-plus-circle"></i> {{ __('messages.new') }}
                                </a>
                            @endif
                        </div>
                    </div>
            ...
            <div class="card-body">
                <div class="row justify-content-between gy-3">
                    <div class="col-md-6 col-lg-4 col-xl-3">
                        <div class="col-md-12">
                            <form action="{{ route('product.bulk-action') }}" id="quick-action-form" class="form-disabled d-flex gap-3 align-items-center">
                                @csrf
                                ...
                                <button id="quick-action-apply" class="btn btn-primary" data-ajax="true" data--submit="{{ route('product.bulk-action') }}" data-datatable="reload" data-confirmation='true' data-title="{{ __('messages.product') }}" title="{{ __('messages.product') }}" data-message='{{ __('Do you want to perform this action?') }}'>{{ __('messages.apply') }}</button>
                        </div>
                        </form>
                    </div>
                    <div class="col-md-6 col-lg-4 col-xl-3">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', (event) => {
            const zone_id = @json($zone_id ?? null);
            window.renderedDataTable = $('#datatable').DataTable({
                ...
                ajax: {
                    "type": "GET",
                    "url": '{{ route('product.product-index-data') }}',
                    "data": function(d) {
                ...
                columns: [
                    { name: 'check', data: 'check', title: '<input type="checkbox" class="form-check-input" name="select_all_table" id="select-all-table" data-type="product" onclick="selectAllTable(this)">', exportable: false, orderable: false, searchable: false },
                    ...
                    { data: 'category_id', name: 'category_id', title: "{{ __('messages.category') }}" },
                    { data: 'price', name: 'price', title: "{{ __('messages.price') }}" },
                    { data: 'status', name: 'status', title: "{{ __('messages.status') }}" },
                    { data: 'action', name: 'action', orderable: false, searchable: false, title: "{{ __('messages.action') }}", className: 'text-end' }
                ],
                order: [[1, 'desc']],
        ...
        $(document).on('click', '[data-ajax="true"]', function(e) {
            ...
            if (button.data('confirmation') === 'true') {
                if (confirm(button.data('message'))) {
                    button.closest('form').attr('action', button.data('submit')).submit();
                }
            } else {
                button.closest('form').attr('action', button.data('submit')).submit();
            }
