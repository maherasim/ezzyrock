                ...
                <div class="col-lg-8 mt-lg-0 mt-5">
                    <div class="tab-content h-100">
                        @php 
                            ...
                            $itemsAll = isset($items) ? $items->values() : collect();
                        @endphp
                        ...
                        @if(($itemType ?? 'service') === 'service')
                            ...
                            @endif
                        @else
                            @php $allRows = $itemsAll->take(6); @endphp
                            @if($allRows->isNotEmpty())
                                <div class="row g-3">
                                    @foreach($allRows as $data)
                                        <div class="col-xl-4 col-md-6">
                                            @if(($itemType ?? '') === 'ecommerce')
                                                @include('product.datatable-card', ['data' => $data, 'cartRow' => ($productCartById[$data->id] ?? null)])
                                            @else
                                                @include('post.datatable-card')
                                            @endif
                                        </div>
                                    @endforeach
                                </div>
                        ...
                        @foreach($sub_category as $subCategory)
                            @php
                                $filteredServices = array_filter($service, fn ($item) => $item->subcategory_id == $subCategory->id);
                                $filteredItems = ($itemsAll ?? collect())->where('subcategory_id', $subCategory->id)->take(6)->values();
                            @endphp
                    ...
                    @if(($itemType ?? 'service') === 'service' && count($service) > 6)
                        <div class="text-center mt-5">
                            <a href="{{route('service.list', ['category_id' => $category->id])}}" class="btn btn-primary text-capitalize">{{__('messages.view_all')}}</a>
                        </div>
                    @elseif(($itemType ?? '') === 'ecommerce' && ($itemsAll->count() ?? 0) > 6)
                        <div class="text-center mt-5">
                            <a href="{{route('product.list', ['category_id' => $category->id])}}" class="btn btn-primary text-capitalize">{{__('messages.view_all')}}</a>
                        </div>
                    @elseif(($itemType ?? '') === 'classified' && ($itemsAll->count() ?? 0) > 6)
                        <div class="text-center mt-5">
                            <a href="{{route('post.list', ['category_id' => $category->id])}}" class="btn btn-primary text-capitalize">{{__('messages.view_all')}}</a>
                        </div>
                    @endif
                </div>
            </div>
