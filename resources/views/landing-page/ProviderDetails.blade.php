                            ...
                            <div class="mt-3 text-center position-relative">
                                ...
                                <div class="d-flex align-items-center justify-content-center gap-1 mt-2">
                                    ...
                                    @if ($providerData['data']['total_service_rating'] > 0)
                                        <div>
                                            ...
                                            @endphp
                                            @for ($i = 0; $i < 5; $i++)
                                                <span class="fa-stack me-1" style="width:1em">
                                ...
                                @if (isset($why_choose_me))
                                    @if (isset($why_choose_me['about_description']) || isset($why_choose_me['reason']))
                                        <div class="mt-2">
                                            <a href="javascipt:void(0);" class="btn btn-primary fw-bold"
                                        ...
                                        <tbody>
                                            <tr>
                                                ...
                                                <td class="text-end pe-0">
                                                    <a href="mailto:{{ $providerData['data']['email'] }}"
                                                        class="text-white">{{ $providerData['data']['email'] }}</a>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td class="px-0">
                                                    <h6 class="text-white m-0 lh-base">{{ __('landingpage.bookings') }}:
                                                    </h6>
                                                </td>
                                                <td class="text-end pe-0">
                ...
                <div class="col-lg-8 mt-lg-0 mt-5">
                            ...
                            <tbody>
                                <tr>
                                    <td class="py-3 px-0 border-top border-bottom">
                                        <h5 class="m-0 text-capitalize font-size-18">{{ __('landingpage.member_since') }}
                                        </h5>
                                    </td>
                                    <td class="py-3 border-top border-bottom">
                                        <span
                                            class="font-size-18">{{ date("$datetime->date_format", strtotime($providerData['data']['created_at'])) }}</span>
                                    </td>
                                </tr>
                                @if (!empty($providerData['data']['known_languages']))
                                    <tr>
                                        <td class="py-3 px-0 border-bottom">
                                            <h5 class="m-0 text-capitalize font-size-18">{{ __('landingpage.languages') }}
                                            </h5>
                                        </td>
                                        <td class="py-3 border-bottom">
                                            <span
                                                class="font-size-18">{{ implode(', ', json_decode($providerData['data']['known_languages'])) }}</span>
                                        </td>
                                    </tr>
                                @endif
                                @if (!empty($providerData['data']['designation']))
                                    <tr>
                                        <td class="py-3 px-0 border-bottom">
                                            <h5 class="m-0 text-capitalize font-size-18">
                                                {{ __('landingpage.designation') }}</h5>
                                        </td>
                                        <td class="py-3 border-bottom">
                    ...
                    @if (!empty($shoplist) && count($shoplist) > 0)
                        <div class="row align-items-center pt-3 mb-5">
                            <div class="col-sm-9">
                                <h4 class="text-capitalize mb-0">{{ __('messages.shops') }}</h4>
                            </div>
                            <div class="col-sm-3 mt-sm-0 mt-3 text-sm-end">
                                <a
                                    href="{{ route('shop.list', ['provider_id' => $providerData['data']['id']]) }}">{{ __('messages.view_all') }}</a>
                            </div>
                            <div
                                class="swiper-container swiper-container-initialized swiper-container-horizontal swiper-container-pointer-events swiper-container">
                                            ...
                                            <div class="mt-5 justify-content-center service-slide-items-4">
                                                <div class="col p-3">
                                                    <div class="service-box-card bg-light rounded-3">
                                                        ...
                                                        <a href="/" class="service-heading mt-2 d-block p-0">
                                                            <h5 class="service-title font-size-18 line-count-2"><a
                                                                    href="{{ route('shop.detail', $shop->id) }}">{{ $shop->shop_name }}</a>
                                                            </h5>
                                                        </a>
                                                        <!-- <div class="mt-3">
                                                            <div class="d-flex align-items-center gap-2">
                                                                <img src="{{ getSingleMedia($shop->provider, 'profile_image', null) }}" alt="service" class="img-fluid rounded-3 object-cover avatar-24">
                                                                <a href="{{ route('provider.detail', $shop->provider->id) }}"><span class="font-size-16 service-user-name">{{ $shop->provider->display_name }}</span></a>
                                                            </div>
                                                        </div> -->
                                                        <div class="mt-3">
                                                            <div class="text-muted">
                                                                <div class="d-flex align-items-center mb-2">
                                                                    ...
                                                                    @php
                                                                        $sitesetup = App\Models\Setting::where(
                                                                            'type',
                                                                            'site-setup',
                                                                        )
                                                                            ->where('key', 'site-setup')
                                                                            ->first();
                                                                        $sitesetupValue = json_decode(
                                                                            optional($sitesetup)->value ?? '{}',
                                                                        );
                                                                        $targetTimezone = isset(
                                                                            $sitesetupValue->time_zone,
                                                                        )
                                                                            ? trim((string) $sitesetupValue->time_zone)
                                                                            : 'UTC';
                                                                        $timeFormat =
                                                                            $sitesetupValue->time_format ?? 'H:i';
                                                                        $sourceTimezone = 'UTC';
                                                                ...
                                                                <div class="d-flex align-items-center">
                                                                    <span>
                                                                        <i
                                                                            class="fas fa-map-marker-alt me-2 text-primary"></i>
                    ...
                    @if (default_earning_type() === 'commission' ||
                            (default_earning_type() === 'subscription' && $providerData['data']['is_subscribe'] == 1))
                        @if (!empty($providerData['service']))
                            ...
                            <service-list-section :user_id="{{ json_encode($auth_user_id) }}"
                                ...
                                :max_records="6" :favourite="{{ json_encode($favourite) }}"
                        ...
                        @if ($providerProducts->isNotEmpty())
                            <div class="row align-items-center pt-3 mb-3">
                                <div class="col-sm-9">
                                    <h4 class="text-capitalize mb-0">{{ __('messages.products') }}</h4>
                                </div>
                                <div class="col-sm-3 mt-sm-0 mt-3 text-sm-end">
                                    <a
                                        href="{{ route('product.list', ['provider_id' => $providerData['data']['id']]) }}">{{ __('messages.view_all') }}</a>
                                </div>
                            </div>
                            <div class="row our-service">
                                @foreach ($providerProducts as $data)
                                    <div class="col-md-6 mb-4">
                                        @include('product.datatable-card', [
                                            'data' => $data,
                                            'cartRow' => $productCartById[$data->id] ?? null,
                                        ])
                                    </div>
                                @endforeach
                            </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            const swiper = new Swiper('.swiper-container', {
                ...
                spaceBetween: 8,
                slidesPerView: 2,
                allowTouchMove: false, // 🚫 Disable swipe/drag
                simulateTouch: false, // 🚫 No mouse dragging
                breakpoints: {
                    0: {
                        slidesPerView: 1,
                    },
