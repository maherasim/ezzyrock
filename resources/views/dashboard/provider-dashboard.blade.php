                                ...
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col">
                                            <div class="d-flex flex-wrap justify-content-start align-items-center">
                                                <h4 class="mb-2 booking-text fw-bold">
                                                    {{ !empty($data['dashboard']['count_total_product']) ? $data['dashboard']['count_total_product'] : 0 }}
                                                </h4>
                                            </div>
                                            <p class="mb-0 booking-text">
                                                {{ __('messages.total_name', ['name' => __('messages.product')]) }}</p>
                                        </div>
                                        <div class="col-auto d-flex flex-column">
                                            <div
                                                class="iq-card-icon iq-card-icon-service icon-shape  text-white rounded-circle shadow">
                            ...
                            <div class="card total-provider-card">
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col">
                                            <div class="d-flex flex-wrap justify-content-start align-items-center">
                                                <h4 class="mb-2 booking-text fw-bold">{{ !empty($data['dashboard']['count_total_provider']) ? $data['dashboard']['count_total_provider'] : 0 }}</h4>
                                ...
                                <li style="pointer-events:none;">
                                    <div class="media gap-3">
                                        ...
                                        <div class="media-body ">
                                            <h5 class="mb-1"><span
                                                    class="fw-bold">{{ !empty($customer->display_name) ? $customer->display_name : '-' }}</span>
                                            </h5>

                                            <span>
                                                {{ optional($data['datetime'])->date_format && optional($data['datetime'])->time_format
                                                    ? \Carbon\Carbon::parse($customer->created_at)->format(optional($data['datetime'])->date_format) .
                                                        '  ' .
                                                        \Carbon\Carbon::parse($customer->created_at)->format(optional($data['datetime'])->time_format)
                                                    : '' }}
                                            </span>
                                        </div>
                                    </div>
                            ...
                            @foreach ($data['dashboard']['upcomming_booking'] as $booking)
                                @php
                                    $getAttachments = getAttachments(
                                        optional($booking->service)->getMedia('service_attachment'),
                                        null,
                                    );
                                @endphp
                                <li class="d-flex flex-wrap gap-2 align-items-start align-items-lg-center justify-content-between  flex-column flex-lg-row"
                                    ...
                                    <div class="media align-items-center gap-3">
                                        ...
                                        <div class="media-body ">
                                            ...
                                            <span>{{ optional($data['datetime'])->date_format && optional($data['datetime'])->time_format
                                                ? date(optional($data['datetime'])->date_format, strtotime($booking->date)) .
                                                    '  ' .
                                                    date(optional($data['datetime'])->time_format, strtotime($booking->date))
                                                : '' }}</span>
                                        </div>
                                    </div>

                                    <span
                                        class="badge rounded-pill py-2 px-3 bg-primary-subtle text-capitalize">{{ ucwords(str_replace('_', ' ', $booking->status)) }}</span>
                                </li>
                            @endforeach
                        </ul>
                    </div>
    ...
    if (jQuery('#monthly-revenue').length) {
        ...
        var options = {
            ...
            yaxis: {
                ...
                labels: {
                    formatter: function(value) {
                        const formattedValue = (typeof value === 'number' && !Number.isInteger(value)) ? value
                            .toFixed(2) : value;
                        return currencyPosition === 'left' ? currencySymbol + formattedValue : formattedValue +
                            currencySymbol;
                    }
                }
            },
            tooltip: {
                y: {
                    formatter: function(value) {
                        return currencyPosition === 'left' ? currencySymbol + value : value + currencySymbol;
                    }
                }
            }
        };
