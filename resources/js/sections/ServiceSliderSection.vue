<template>
  <section ref="servicesection">
    <Swiper class="swiper-container" v-if="serviceDetails.length > 0" :modules="modules" :slides-per-view="6"
      :space-between="20" :pagination="{ clickable: true }" :loop="false"
    ...
    <Swiper class="swiper-container" v-if="IS_LOADER == true && serviceDetails.length == 0" :modules="modules"
      :slides-per-view="6" :space-between="20" :pagination="{ clickable: true }" :loop="true"
      :autoplay="{ delay: 3000, disableOnInteraction: false }" :breakpoints="{
        320: { slidesPerView: 1, spaceBetween: 16 },
        576: { slidesPerView: 2, spaceBetween: 20 },
        768: { slidesPerView: 3, spaceBetween: 20 },
...
/** Top Rated (ac) + Featured (cleaning): max 6 in one row on desktop */
const MAX_LANDING_SECTION_SERVICES = 6;
const capLandingServices = (list) => {
  if (props.type === 'ac' || props.type === 'cleaning') {
    return list.slice(0, MAX_LANDING_SECTION_SERVICES);
  }
  return list;
};
  ...
  try {
    ...
    if (settings) {
      ...
      if (earningtype === 'subscription') {
        ...
        serviceDetails.value = capLandingServices(filteredServices);
      } else {
        ...
        serviceDetails.value = capLandingServices(uniqueServices);
      }
