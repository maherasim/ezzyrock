<template>
   ...
   <div class="bg-body search-box-wrapper" v-for="setting in section1Data" :key="setting.id">
      <div class="d-flex align-items-center justify-content-between"
         v-if="setting.key === 'section_1' && getJsonValue(setting.value, 'current_location') == 'on' || getJsonValue(setting.value, 'enable_search') == 'on'">
         <div class="d-flex location-search-block align-items-center">
            <div v-if="setting.key === 'section_1' && getJsonValue(setting.value, 'current_location') == 'on'"
               class="p-3 d-inline-flex align-items-center gap-2 f-none modal-location" data-bs-toggle="modal"
               ...
               <span class="font-size-14 text-body text-capitalize" style="cursor: pointer;"
                  v-if="setting.key === 'section_1' && getJsonValue(setting.value, 'current_location') === 'on'">{{ Location_name ??
                     $t('landingpage.current_location') }}</span>
            </div>
            <div class="p-3 location-search">
               <div class="form-group input-group mb-0 ms-0 ps-3 border-start"
                  v-if="setting.key === 'section_1' && getJsonValue(setting.value, 'enable_search') == 'on'">
                  ...
                  <input type="text" class="font-size-14 bg-transparent border-0"
                     :placeholder="$t('landingpage.search_service')" @keyup="getServiceList" v-model="keyword">
                  <div
                     :class="`top-width dropdown-menu dropdown-menu-end user-dropdown mt-5 ${keyword.length > 0 ? 'show' : ''}`"
                        ...
                        <div v-if="serviceList.length > 0 && !show">
                           <div class="searchbox-datalink global-search-result show-data">
                              <ul class="iq-post p-3">
                                 <li v-for="service in serviceList" :key="`${service.module || 'service'}-${service.id}`" class="mr-0 mb-3 pb-0 d-block">
                                    <a :href="service.url || `${baseUrl}/service-detail/${service.id}`">
                                       <div class="post-img d-flex align-items-center">
                                          <div class="post-img-holder">
                                             <a :href="service.url || `${baseUrl}/service-detail/${service.id}`">
                                                <img
                                                   :src="service.image || (service.attchments && service.attchments[0] ? service.attchments[0] : baseUrl + '/images/default.png')"
                                                   alt="">
                                             </a>
                                          </div>
                                          <div class="post-blog pl-2">
                                             <div class="blog-box">
                                                ...
                                                <small class="text-muted text-uppercase d-block">{{ service.module || 'service' }}</small>
                                             </div>
                                          </div>
                                       </div>
                                    </a>
         ...
         <div class="modal-dialog">
            <div class="modal-content">
               <div class="modal-header">
                  <h5 class="modal-title text-capitalize" id="modal-locationLabel">
                     {{ $t('landingpage.get_nearest_services') }}</h5>
               ...
               <div class="modal-body">
                  <form>
                     <p class="small text-muted mb-2">{{ $t('landingpage.services_near_you_india_hint') }}</p>
                     <p v-if="user_lat != null && user_lng != null">{{ $t('landingpage.continue_view_services') }}</p>
                     <p v-else>{{ $t('landingpage.discover_find_services') }}</p>
                     <div class="mb-3">
                        <label class="form-label">{{ $t('landingpage.select_zone_manual') }}</label>
                        <select ref="manualZoneRef" class="form-select" v-model="manualZoneId">
                           <option value="">{{ $t('landingpage.choose_zone') }}</option>
                           <optgroup v-for="group in groupedZoneOptions" :key="group.label" :label="group.label">
                              <option v-for="zone in group.items" :key="zone.id" :value="String(zone.id)">
                                 {{ zone.name }}
                              </option>
                           </optgroup>
                        </select>
                        <small class="text-muted d-block mt-1">{{ $t('landingpage.manual_zone_hint') }}</small>
                     </div>
                     <div class="row row-cols-1 g-2 pt-1 mb-3">
                        <div class="col">
                           <button type="button" class="btn btn-outline-primary w-100" :disabled="!manualZoneId" @click="applyManualZone">
                              {{ $t('landingpage.use_selected_zone') }}
                           </button>
                        </div>
                     </div>
                     <div class="row row-cols-2 pt-2">
                        <div class="col">
                           <button type="button" class="btn btn-secondary w-100" data-bs-dismiss="modal"
                              aria-label="Close">{{ $t('messages.cancel') }}</button>
                        </div>
                        <div class="col">
                           <button type="button" class="btn btn-primary w-100" @click="getServices">{{
                              $t('landingpage.use_current_location') }}</button>
                        </div>
                        <div v-if="user_lat != null && user_lng != null" class="col-12">
                           <button type="button" class="btn btn-outline-danger w-100" @click="removeCurrentLocation">{{
                              $t('messages.off_location') }}</button>
                        </div>
   ...
   <div class="mt-3 d-flex align-items-center justify-content-between gap-2 flex-wrap">
      <div v-for="setting in section1Data" :key="setting.id">
         <ul class="p-0 list-inline categories-list mb-0"
            v-if="setting.key === 'section_1' && getJsonValue(setting.value, 'enable_popular_services') == 'on'">
            <li class="d-inline-block text-capitalize font-size-14 position-relative p-1">
               <h6>{{ getJsonValue(setting.value, 'title') }}</h6>
            </li>
            <li v-for="(categoryIds, categoryIndex) in getJsonValue(setting.value, 'category_id')" :key="categoryIndex"
               class="d-inline-block text-capitalize font-size-14 position-relative p-1">
               <a v-if="getCategoryNames([categoryIds]) != ''" :href="`${baseUrl}/category-details/` + categoryIds"
            ...
            <div class="modal-content">
               <div class="modal-header justify-content-center">
                  <div class="text-center">
                     <h5 class="modal-title text-capitalize" id="modal-locationLabel">{{
                        $t('landingpage.add_your_service') }}</h5>
                     ...
                     <div class="row">
                        ...
                        <div class="mb-4 col-md-6">
                           ...
                           <input v-model="serviceName" type="text" class="form-control"
                              placeholder="Write Service Name" aria-label="servicename" aria-describedby="basic-addon1">
                        ...
                        <div class="mb-4 col-md-6">
                           ...
                           <input v-model="price" type="number" class="form-control" placeholder="Price"
                              aria-label="price" aria-describedby="basic-addon1" min="1">
                        ...
                        <div class="mb-4 col-md-12">
                           ...
                           <textarea v-model="serviceDescription" class="form-control" rows="4"
                              placeholder="Write Here..."></textarea>
                        </div>
                        <div class="mb-4">
                           <button type="submit" class="btn btn-primary" :disabled="isLoading">{{
                              $t('messages.submit') }}</button>
                        </div>
                     </div>
                  </form>
               </div>
...
import { CATEGORY_API, SEARCH_API, POST_SERVICE_API, REMOVE_SESSION, SET_SESSION, ZONES_API } from '../data/api';
import { useSection } from '../store/index'
//import {useObserveSection} from '../hooks/Observer'
const props = defineProps({
   postjobservice: { type: [String, Number], default: 0 },
   user_id: { default: null },
   user_lat: { type: [String, Number], default: '' },
   user_lng: { type: [String, Number], default: '' },
...
const Location_name = ref(props.locationLabel || localStorage.getItem('loction_name') || null);
const zoneOptions = ref([]);
const manualZoneId = ref('');
const manualZoneRef = ref(null);

const inferStateLabel = (zoneName) => {
   const name = String(zoneName || '').trim();
   if (!name) return 'Other';
   const parts = name.split(',').map((p) => p.trim()).filter(Boolean);
   return parts.length >= 2 ? parts[parts.length - 1] : 'Other';
};

const groupedZoneOptions = computed(() => {
   const groups = {};
   zoneOptions.value.forEach((zone) => {
      const key = inferStateLabel(zone.name);
   ...
   return Object.keys(groups)
      ...
      .map((label) => ({
         label,
         items: groups[label].slice().sort((a, b) => String(a.name).localeCompare(String(b.name))),
      }));
});

const parseZoneCoordinates = (coordinates) => {
   ...
   return Array.isArray(coords) ? coords.filter((c) => c && c.lat != null && c.lng != null) : [];
};

const zoneCentroid = (coordinates) => {
   const pts = parseZoneCoordinates(coordinates);
...
const loadZones = async () => {
   try {
      const response = await fetch(ZONES_API({ per_page: 'all' }));
      const payload = await response.json();
      const rows = payload?.data?.data ?? payload?.data ?? [];
      zoneOptions.value = Array.isArray(rows) ? rows : [];
      await nextTick();
      if (manualZoneRef.value && window.jQuery && jQuery.fn.select2) {
         jQuery(manualZoneRef.value)
            .select2({
               width: '100%',
               placeholder: 'Choose a zone',
               allowClear: true,
            })
            .off('change.manualZone')
            .on('change.manualZone', function () {
               manualZoneId.value = String(jQuery(this).val() || '');
            });
      }
   } catch (error) {
      zoneOptions.value = [];
...
const applyManualZone = async () => {
   const selected = zoneOptions.value.find((z) => String(z.id) === String(manualZoneId.value));
   if (!selected) return;
   const centroid = zoneCentroid(selected.coordinates);
   if (!centroid) return;

   localStorage.setItem('loction_current_lat', String(centroid.lat));
   localStorage.setItem('loction_current_long', String(centroid.lng));
   localStorage.setItem('is_location_on', 'on');
   localStorage.setItem('loction_name', selected.name);
   Location_name.value = selected.name;
   ...
   const response = await fetch(`${SET_SESSION}?${params.toString()}`);
   const data = await response.json();
      ...
      navigator.geolocation.getCurrentPosition(
         async (position) => {
            ...
            localStorage.setItem('is_location_on', 'on');
              ...
              const geo = await getLocationName(currentLatitude, currentLongitude);
              if (geo && geo.label) {
                 Location_name.value = geo.label;
              }

              const params = new URLSearchParams({
                 lat: String(currentLatitude),
                 lng: String(currentLongitude),
                 label: (geo && geo.label) ? geo.label : '',
                 city: (geo && geo.city) ? geo.city : '',
                 state: (geo && geo.state) ? geo.state : '',
              });
              const response = await fetch(`${SET_SESSION}?${params.toString()}`);
                 const data = await response.json();
            ...
            if (typeof showofflocation !== "undefined") {
         ...
         (error) => {
            ...
            // Set fallback values or just skip location-based filtering
          localStorage.setItem('is_location_on', 'off');

            // Optional: show message to user
            // Fallback URL without location
            const serviceUrl = `${baseUrl}/service-list`;
            window.location.href = serviceUrl;
...
const getLocationName = async (latitude, longitude) => {
   ...
   try {
      ...
      localStorage.setItem('loction_name', label);
      return { label, city, state };
   } catch (error) {
...
const removeCurrentLocation = async () => {
   try {
      // Remove location values from localStorage
      localStorage.setItem('loction_current_lat','');
      localStorage.setItem('loction_current_long','');
      localStorage.setItem('is_location_on', 'off');
      localStorage.setItem('loction_name', '');
...
const getServiceList = async () => {
   try {
      if (keyword.value.trim().length >= 2) {
         const response = await fetch(SEARCH_API({ include_modules: 1, search: keyword.value }));
         const data = await response.json();
         if (data && Array.isArray(data.data)) {
            show.value = false;
