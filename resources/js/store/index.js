...
import { PROVIDER_API,BOOKINGLIST_API,POSTJOBLIST_API,BOOKINGSTATUS_API,PROMOTIONAL_BANNER_API } from '../data/api';
...
export const useSection = defineStore('section', {
    ...
    getters: {
        ...
        post_job_list_data: (state) => state.post_job_list,
        promotional_banners_data: (state) => state.promotional_banners.data,

      },
    actions:{
        async get_categries_list({ per_page, page, module_type }){
            const searchData = { per_page: per_page, page: page }
            if (module_type) {
                searchData.module_type = module_type
            }
        ...
        async get_post_job_list({per_page}){
          const searchData = { per_page: per_page }
          const response = await axios.get(POSTJOBLIST_API(searchData))
          this.post_job_list = response.data
      },
       ...
       async get_promotional_banner({per_page}){
              const searchData = { per_page: per_page }
              const response = await axios.get(PROMOTIONAL_BANNER_API(searchData))
              this.promotional_banners = response.data
          },
